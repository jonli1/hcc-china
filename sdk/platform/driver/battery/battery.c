...
static uint16_t batt_cal_volt_cr2032(void)
{
    ...
    adc_config_t adc_cfg =
    {
        ...
        .input            = ADC_INPUT_SE_P0_6,//ADC_INPUT_SE_VBAT_HIGH,
        .smpl_time_mult   = 2,
        .continuous       = false,
        .interval_mult    = 0,
...
static uint8_t batt_cal_cr2032(uint16_t batt_volt)
{
    ...
    {
        return (((batt_volt - 2230) * 100) / (2770 - 2230));
    }
	}else{
		if (batt_volt < 2100)
			{
			...
			{
					return (((batt_volt - 2100) * 100) / (2610 - 2100));
			}
	}
#endif
}
